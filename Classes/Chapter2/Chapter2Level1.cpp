...
bool Chapter2Level1::init()
{
	...
	hero = Sprite::create("chapter2/Blue_Front1.png");
	hero->setAnchorPoint(Vec2(0, 0));
	hero->setPosition(10, 100);
	hero->setName("Hero");

	auto physicsBodyHero = PhysicsBody::createBox(hero->getContentSize(), PhysicsMaterial(0.1f, 0.0f, 0.0f));
	physicsBodyHero->setGravityEnable(true);
	physicsBodyHero->setRotationEnable(false);
	//physicsBodyHero->setCategoryBitmask(0x01);    // 0001
	//physicsBodyHero->setCollisionBitmask(0x06);   // 0110
	hero->setPhysicsBody(physicsBodyHero);
	hero->setTag(1);
	...
	hero->getPhysicsBody()->applyForce(Vec2(30000000,0));
...
bool Chapter2Level1::onContactBegin(const cocos2d::PhysicsContact &contact) {
	...
	//log("shapea: %d shapeb: %d", nodeA->getTag(), nodeB->getTag());
...
void Chapter2Level1::heroRight() {
	if (heroState != 1) {
		auto animation = Animation::create();
		for (int i = 1; i <= 3; i++)
			animation->addSpriteFrameWithFile("chapter2/Blue_Right" + std::to_string(i) + ".png");
...
void Chapter2Level1::heroLeft() {
	if (heroState != 2) {
		auto animation = Animation::create();
		for (int i = 1; i <= 3; i++)
			animation->addSpriteFrameWithFile("chapter2/Blue_Left" + std::to_string(i) + ".png");
		animation->setDelayPerUnit(0.15f);
		hero->stopAllActions();
		hero->runAction(RepeatForever::create(Animate::create(animation)));
		heroState = 2;
...
void Chapter2Level1::heroJump() {
	if (heroState != 3) {
		auto animation = Animation::create();
		for (int i = 1; i <= 3; i++)
			animation->addSpriteFrameWithFile("chapter2/Blue_Front" + std::to_string(i) + ".png");
		animation->setDelayPerUnit(0.15f);
		hero->stopAllActions();
		hero->runAction(RepeatForever::create(Animate::create(animation)));
		heroState = 3;
...
void Chapter2Level1::heroSilence() {
	hero->stopAllActions();
	hero->setTexture("chapter2/Blue_Front1.png");
	heroState = 0;
}